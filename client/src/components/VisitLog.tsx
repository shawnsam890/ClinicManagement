...
import SimplifiedVisitDetail from './SimplifiedVisitDetail';
...
export default function VisitLog({ 
  ...
  const [activeVisit, setActiveVisit] = useState(visits.find(v => v.id === selectedVisitId) || null);
  ...
  const getChiefComplaint = (visit) => visit.chiefComplaint || "Unknown";
  
  const handleVisitClick = (visitId) => {
    onViewVisit(visitId);
    const clickedVisit = visits.find(v => v.id === visitId);
    setActiveVisit(clickedVisit || null);
  };
  ...
  return (
    <>
      {activeVisit && (
        <div className="mb-6">
          <SimplifiedVisitDetail visit={activeVisit} />
        </div>
      )}
